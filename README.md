...
├── office_com/               ← MS Office COM 自动化（独立模块）
│   ├── _base.py
│   ├── word_app.py
│   ├── excel_app.py
│   └── ppt_app.py
...
## office_com — MS Office COM 自动化

独立模块，不依赖 ArcGIS。Word/Excel 的 PDF 导出和格式转换可用，PPT 不可用于自动化。

```python
from office_com import WordApp, ExcelApp
